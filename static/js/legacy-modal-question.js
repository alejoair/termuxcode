...
export function showAskUserQuestion(questions, tabId, ws) {
    ...
    const overlay = createOverlay('questionOverlay');
...
export function hideAskUserQuestion(tabId) {
    hideModal(questionModals, tabId);
}

// Migrar estado de modal cuando el tab cambia de ID (re-key por session_id)
export function migrateQuestionModal(oldTabId, newTabId) {
    if (questionModals.has(oldTabId)) {
        const modalState = questionModals.get(oldTabId);
        questionModals.delete(oldTabId);
        modalState.tabId = newTabId;
        questionModals.set(newTabId, modalState);
    }
}
